...
def test_build_workbook_shades_rows_by_reporting_month(tmp_path):
    ...
    # Rows are sorted by reporting_month before writing, so data rows 3-4
    # (row 1 is the title, row 2 the header) are both January (same shading
    # color) and row 5 is February (next color in the MONTH_FILL_COLORS
    # cycle).
    jan_fill_a = ws.cell(row=3, column=1).fill.fgColor.rgb
    jan_fill_b = ws.cell(row=4, column=1).fill.fgColor.rgb
    feb_fill = ws.cell(row=5, column=1).fill.fgColor.rgb
...
def test_build_workbook_sorts_by_receive_date_within_reporting_month(tmp_path):
    ...
    assert [ws.cell(row=r, column=vend_col).value for r in (3, 4, 5)] == ["First", "Second", "Third"]
...
def test_build_workbook_attaches_table_with_correct_headers(tmp_path):
    ...
    assert table.ref == f"A2:{ws.cell(row=2, column=len(HEADER)).column_letter}3"
    assert slicer_info[WITHIN_SHEET] == (1, HEADER.index("reporting_month") + 1, len(HEADER))


def test_build_workbook_adds_merged_title_row_above_header(tmp_path):
    wb, _, _ = _build_and_reload(tmp_path, {WITHIN_SHEET: [_row()], OUTSIDE_SHEET: [_row()]})

    for sheet_name, expected_title in ((WITHIN_SHEET, WITHIN_TITLE), (OUTSIDE_SHEET, OUTSIDE_TITLE)):
        ws = wb[sheet_name]
        title_cell = ws.cell(row=1, column=1)
        assert title_cell.value == expected_title
        assert title_cell.font.bold is True
        assert title_cell.font.size == TITLE_FONT.size
        ...
        # Header row itself is unaffected by the title above it.
...
def test_build_workbook_leaves_ordinary_text_untouched(tmp_path):
    ...
    assert ws.cell(row=3, column=HEADER.index("vend") + 1).value == "Acme Co"
    assert ws.cell(row=3, column=HEADER.index("po") + 1).value == "PO123"
...
def test_build_workbook_sizes_columns_to_longest_value(tmp_path):
    ...
    vend_letter = ws.cell(row=2, column=HEADER.index("vend") + 1).column_letter
    dept_letter = ws.cell(row=2, column=HEADER.index("dept") + 1).column_letter
...
def test_build_workbook_handles_empty_bucket_without_error(tmp_path):
    ...
    assert ws.cell(row=1, column=1).value == WITHIN_TITLE
    assert ws.cell(row=2, column=1).value == "reporting_month"
    assert "Table1" in ws.tables
    ...
    assert ws_outside.max_row == 3  # title + header + 1 data row
...
def test_build_workbook_without_on_progress_behaves_unchanged(tmp_path):
    ...
    wb, _, _ = _build_and_reload(tmp_path, {WITHIN_SHEET: [_row()]})
    assert wb[WITHIN_SHEET].max_row == 3  # title + header + 1 data row
